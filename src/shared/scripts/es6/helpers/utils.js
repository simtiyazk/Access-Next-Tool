export default () => {
   const self = {};
   const touchclick = ('ontouchend' in document.documentElement) ? 'touchend' : (window.navigator.pointerEnabled) ? 'pointerup' : 'click'; // Handles Tap or click events
   ...
   function overrideLinks() {
      $('a[href^="veeva:gotoSlide("]').on(touchclick, function (e) {
         ...
         if (self.isVeeva === true) {
            ...
            setTimeout(function () { location.href = `../${keyMessage}/${keyMessage}.html`; }, delay);
         }
      });
   }
