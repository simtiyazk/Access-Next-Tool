$(document).ready(function () {
  ...
  let controlCenter = JSON.parse(localStorage.getItem('controlCenter'));

  localStorage.removeItem('controlCenter');
  localStorage.removeItem('edititem');
  localStorage.removeItem('storeValue');

  var plansSelected = [], isPlansSelected = true, isSaved = true;
      ...
      if (result.success == true) {
        ...
        function getCountybyFipscode() {
          if (result.success == true) {
            ...
            function getJSONpayloadbyFipscode() {
              if (result.success == true) {
                let resultArr = result.AN_County__c[0].JSON_Payload__c;

                let resArr = resultArr.replace(/\\/, "")
  ...
  function initialSetup() {
    ...
    if (storeValue && storeValue != undefined) { // Handle for - if user selecting the product on access_next_tool_20_00 to set the dropdown value
      let selectedProd = $(".productDD > span");
    ...
    if (editItemId >= 0 && editItemId != undefined) {
      let localTemplates = JSON.parse(localStorage.getItem('savedTemplates'));
      let prodId, prodText;
      let selectedProd = localTemplates[editItemId].product;
      let selectedChannel = localTemplates[editItemId].channel;
      let selectedBenefit = localTemplates[editItemId].benefittype;
      let selectedAccess = localTemplates[editItemId].access;
      plansSelected = localTemplates[editItemId].plan;

      $.each(jsonResultpayload, function (i, row) {
        if (row.brand == selectedProd) {
          prodId = i;
        }
      });
      ...
      $('.custom-options.productDD > span.custom-option').each(function (index) {
        ...
        if (selectedProd.toUpperCase().match('CUVITRU/HYQVIA')) {
          $('.item-logo>img').attr('src', '../shared/images/takeda_logo.png');
          $('.presentBtn').attr('href', 'veeva:gotoSlide(access_next_tool_70_00.zip)');
        } else if (selectedProd.toUpperCase().match("HYQVIA")) {
          $('.item-logo>img').attr('src', '../shared/images/hyqvia_logo.png');
          $('.presentBtn').attr('href', 'veeva:gotoSlide(access_next_tool_50_00.zip)');
        } else if (selectedProd.toUpperCase().match("GAMMAGARD")) {
          $('.item-logo>img').attr('src', '../shared/images/gammagard_logo.png');
          $('.presentBtn').attr('href', 'veeva:gotoSlide(access_next_tool_60_00.zip)');
        } else if (selectedProd.toUpperCase().match("CUVITRU")) {
          $('.item-logo>img').attr('src', '../shared/images/cuvitru_logo.png');
          $('.presentBtn').attr('href', 'veeva:gotoSlide(access_next_tool_40_00.zip)');
        }
      });
      ...
      $('.custom-options.benefitDD > span.custom-option').each(function (index, row) {
        let benefitId = $(this).attr('benefit-id');
        if (benefitId) {
          if (benefitSelected.benefit == selectedBenefit) {
            ...
            $.each(benefitSelected.payers, function (i, row) {
              ...
              let isActive = '';
              let updateTable = '';
              let accessClicked = $('.custom-options.accessDD > span.custom-option.selected').text();
              let benefitTypeClicked = benefitSelected.benefit;
    ...
    if (controlCenter) {
      let prodId;
      let selectedProd = controlCenter.product;
      let selectedChannel = controlCenter.channel;
      let selectedBenefit = controlCenter.benefittype;
      let selectedAccess = controlCenter.access;
  ...
  function setDropdowns() { //Handle for - To set default values to dropdown
    let channels = $(".channelDD > span");
  ...
  $("body").on("click", "#productDD span", function () {
    $('.product').text(
      $('.custom-options.productDD > span.custom-option.selected').text()
    );
    ...
    let storeValue = $('.custom-options.productDD > span.custom-option.selected').text().toUpperCase();
  ...
  $("body").on("click", "#channelDD span", function () {
    $('.channel').text(
      $('.custom-options.channelDD > span.custom-option.selected').text()
    );
  ...
  $("body").on("click", "#benefitDD span", function () {
    ...
    let productTypeClicked = prodselected.brand;
    let benefitTypeClicked = benefitSelected.benefit;
    let accessClicked = $('.custom-options.accessDD > span.custom-option.selected').text();
  ...
  $("body").on("click", "#accessDD span", function () {
    $('.access').text(
      $('.custom-options.accessDD > span.custom-option.selected').text()
    );
  });
  ...
  $(".cta-white").click(function () {
    ...
    if (editItemId >= 0 && editItemId != undefined) { // edit save template
      var backLabel = savedTemplates[editItemId].label;
      var newTemplates = new addTemplate(
        backLabel,
        product,
        channel,
        access,
    ...
    } else { // save newly created template
  ...
  $('.presentBtn').on('click', function () {
    let product = prodselected.brand;
    let channel = channelSelected.channel;
    let benefittype = (benefitSelected) ? benefitSelected.benefit : 'None';
    let access = $('.custom-options.accessDD > span.custom-option.selected').text();
    var plans = [];
    $.each($(".option.active label"), function () {
      plans.push($(this).text());
    });
...
let accessText = "";
const accessList = ["Covered with restrictions", "Covered without restrictions"];
